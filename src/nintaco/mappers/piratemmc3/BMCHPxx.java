...
public class BMCHPxx extends MMC3 {
  ...
  @Override protected void setChrBank(final int bank, int value) {
    if (getBitBool(regs[0], 2)) {
      switch(regs[0] & 3) {
        case 2:  value = (regs[2] & 0x3E) | (unromChr & 0x01); break;
        case 3:  value = (regs[2] & 0x3C) | (unromChr & 0x03); break;
        default: value = regs[2] & 0x3F;                       break;          
      }
      value <<= 3;
      for(int i = 7; i >= 0; --i) {
  ...
  @Override protected void setPrgBank(final int bank, int value) {
    if (getBitBool(regs[0], 2)) {
      if ((regs[0] & 0x0F) == 0x04) {
        value = (regs[1] & 0x1F) << 1;
  ...
  @Override protected void writeMirroring(final int value) {
    if (getBitBool(regs[0], 2)) {
