...
public class LocalAPI implements API {
  ...
  private RacerMateMapper getRacerMate(int port) {
    final Mapper mapper = _mapper;
    if (mapper == null)
      return null;
    final DeviceMapper[] deviceMappers = mapper.getDeviceMappers();
    if (deviceMappers == null || port < 0 || port >= 2 || port >= deviceMappers.length || deviceMappers[port] == null ||
        deviceMappers[port].getInputDevice() != InputDevices.RacerMate1)
      return null;
    return (RacerMateMapper)deviceMappers[port];
  }
  
  @Override
  @ApiMethod(165)
  public int getRacerMateData(final int port, final int index) {
    RacerMateMapper racerMate = getRacerMate(port);
    if (racerMate == null)
      return -1;
    else
      return racerMate.getData(index);
  }

  @Override
  @ApiMethod(166)
  public void setRacerMateData(final int port, final int index, final int data) {
    RacerMateMapper racerMate = getRacerMate(port);
    if (racerMate == null)
      return;
    racerMate.setData(index, data);
  }
}
