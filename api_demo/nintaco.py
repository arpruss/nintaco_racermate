...
class _RemoteBase(object):
  ...
  def _fireDeactivated(self):
    for listenerID, listener in copy.copy(list(self._listenerObjects[_Deactivate]
        .items())):
      listener()
  
  def _fireStatusChanged(self, message):
    for listenerID, listener in copy.copy(list(self._listenerObjects[_Status]
        .items())):
      listener(message)
...
class _RemoteAPI(_RemoteBase):
  ...
  def getRacerMateData(self,port,index):
    try:
      self._stream.writeByte(165)
      self._stream.writeInt(port)
      self._stream.writeInt(index)
      self._stream.flush()
      return signedInt(self._stream.readInt())
    except:
      pass
    return -1

  def setRacerMateData(self,port,index,data):
    try:
      self._stream.writeByte(166)
      self._stream.writeInt(port)
      self._stream.writeInt(index)
      self._stream.writeInt(data)
      self._stream.flush()
    except:
      pass
